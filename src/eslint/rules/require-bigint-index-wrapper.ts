      ...
      MemberExpression(node) {
        ...
        context.report({
          ...
          message: formatMessage(
            options.lang,
            `genshin-ts项目使用bigint表示运行时整数类型, 但bigint无法作为typescript索引, 为此, 你需要用一个${helperName}()函数进行包裹, 从而通过类型检查, ${helperName}()不产生实际作用, 仅用于通过检查, 你可以直接应用eslint修复自动执行此操作; 此外, 如果你看到此处显示的是警告, 而不是错误, 那么, 通常意味着本项目的typescript插件已经生效, 将bigint视作了正常的索引值, 此时, 你可以禁用gsts/require-bigint-index-wrapper eslint规则; 如果你用的是vscode/cursor这类编辑器, 并且此处显示了一个错误, 提示bigint不可作为索引, 那么, 你可以为编辑器配置 "typescript.tsdk": "node_modules/typescript/lib" 和 "typescript.enablePromptUseWorkspaceTsdk": true (genshin-ts的项目模板已经自带了这些设置), 并设置"使用工作区的typescript版本", 这样你无需使用${helperName}()就可以通过类型检查`,
            `genshin-ts uses bigint for runtime integer values, but bigint cannot be used as a TypeScript index type. Wrap the index with ${helperName}() to pass type-checking. ${helperName}() has no runtime effect and is only for type checking. You can apply ESLint auto-fix directly. If this appears as a warning (not an error), the project TypeScript plugin is likely active and bigint is already treated as a valid index value, so you may disable gsts/require-bigint-index-wrapper. If you use VSCode/Cursor and still see an error that bigint cannot be used as an index type, configure "typescript.tsdk": "node_modules/typescript/lib" and "typescript.enablePromptUseWorkspaceTsdk": true (the genshin-ts project template already includes these settings), then switch to the workspace TypeScript version. In that case, you can pass type-checking without ${helperName}().`
          ),
          fix(fixer) {
