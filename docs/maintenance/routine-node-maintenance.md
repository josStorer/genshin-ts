...
- 若只是新增参数，不要顺手破坏旧签名
- 对已有节点的文案更新，先明确定位到具体旧 API，再逐条替换说明；不要用脚本按资源数组顺序批量覆盖。
- 中文描述不能简单按英文资源节点的 `section index + node index` 去取中文资源，因为新版资源可能出现中英文数组错位。
- 输出参数名不等于中文返回说明。像 `Consumer?` 这类 pin 名应结合类型和 description 翻译成语义，例如 bool 返回值应写成“是否成功消耗礼盒”，不要直译成“消耗者”。
- 事件必须同时更新：
  - `events.ts`
  - `events-payload.ts`
...
6. 对“只有描述变化”的旧节点，先用英文资源名定位到现有 TS API，再人工改英文与中文首段说明；中文内容可以参考现有中文注释、官方中文名和英文新描述手动翻译，但不要直接信任错位的中文资源项。
7. 每次小块修改后运行无产物检查，例如 `npx tsc -p tsconfig.json --noEmit`。

除非本轮目标就是重建整套定义层，否则不要把 `npm run gen` 的输出直接作为最终改动提交。

### 旧 API、资源改名与注释更新

新版资源可能把官方显示名从 `Modify ...` 改成 `Set ...` / `Increase ...`，但这不一定表示 TypeScript API 必须改名。

本轮后续确认的维护策略是：

- 保留旧 TypeScript API 名，避免破坏用户代码。
- 只在 `SPECIAL_NODE_MAPPINGS` 和测试生成器的 vendor id 映射里维护“新资源名 -> 旧 API/vendor 名”。
- 注释可以按新资源描述更新，但不要因为注释更新顺手改方法名。
- `zh_aliases.ts` 继续维护当前对外 API 的中文别名；不要为了资源改名批量新增一批含义重复的别名。

处理疑似改名节点时，要把三件事分开：

1. API 是否改名：默认不改。
2. 编译器 vendor 映射是否需要改：需要按资源名映射到旧 vendor 名。
3. 注释是否需要更新：需要时逐条更新英文和中文说明。
...
本轮后续维护里又确认过一批“不是新增节点，但资源文档已经更新”的旧节点。处理原则是先确认编辑器/资源真值，再只改必要字段。

已按资源文档更新签名和实现的节点：

- `consumeGiftBox`
  - 新增 bool 返回值。
  - 输出 pin 名为 `consumer`，`output index = 0`。
  - 中文返回说明应表达 bool 语义，例如“是否成功消耗礼盒”，不要直译 pin 名。
...
已确认要保留现有封装、不要按资源原样退化的差异：

- 列表节点资源里常把列表参数写成 `Generic`，但项目 API 应保留 `T[]` / `generic_list` 语义。
- 字典节点资源里常把字典参数写成 `Generic`，但项目 API 应保留 `dict<K, V>` / `dict` 语义。
- 这类差异是项目类型系统的封装，不应为了让静态报告“无差异”而降级类型。

已确认属于资源或脚本解析漂移的差异：

- `setCharacterSElementalEnergy` / `increasesCharacterSElementalEnergy`
  - 资源里显示无参数，但编辑器确认经典模式 API 有参数，保留现有签名。
- `sendSignal`
  - 项目用 `signalName` 封装信号名，保留现有签名。
- `querySkillAttributeGroupValue`
  - 资源里参数类型解析为 `unknown`，当前 `configId` 封装更符合项目实现。

### 单返回值 pin 名的检查规则

静态 diff 脚本不能只从 TypeScript 返回类型判断返回参数名。

例如：

- `searchListAndReturnValueId` 的 TS 返回类型是 `bigint[]`，但真实输出 pin 名是 `idList`。
- `queryIfDictionaryContainsSpecificKey` 的 TS 返回类型是 `boolean`，但真实输出 pin 名是 `include`。
...
如果只是临时给用户导入编辑器验证，不一定要写入 `dist/tests`。可以把单文件 `.gia` 输出到 `C:\tmp`，例如：

- `C:\tmp\manual_verify_updated_doc_nodes.gia`

这样可以避免把 build/test 产物混进项目工作区。若最终决定保留验证文件，再把对应 `.ts` 脚本纳入仓库即可。
...
- 同一个 API 已经按模式解析不同 vendor id，却又额外新增了模式专用包装方法，导致重复计数。

本轮后续确认：`teleportPlayer` 已经同时覆盖经典模式和超限模式，编译器按 graph mode 解析不同 vendor id；不要再额外保留 `teleportPlayerClassicMode` 这类经典模式专用包装，否则经典模式节点数量会多算 1。
...
但要注意：官方中文资源数组可能和英文资源数组错位。遇到以下现象时，不能继续按数组下标批量更新中文注释：

- 英文 `Set List Value` 对到中文 `清除列表`
- 英文 `Set Environment Configuration` 对到中文 `复苏玩家所有角色`
- 同一 section 内中文名明显落到相邻节点

这时应改为：

1. 用英文资源名和现有 API 映射定位具体节点。
2. 用现有 `zh_aliases.ts` 和当前中文注释确认中文节点名。
3. 结合英文新描述逐条手工更新中文说明。
4. 更新后用 `rg` 扫描旧中文短语，确认没有遗漏。
...
### 9. 只补新增节点，漏掉旧节点文档或返回值更新

资源更新不一定表现为新增/删除节点。节点数量完全对齐时，仍然可能存在：

- 旧节点返回值新增
- 旧节点输出字段拆分
- 参数名或参数描述更新
- 纯文案说明更新
- 官方显示名从 `Modify` 改成 `Set` / `Increase`

所以每轮都要单独看 `Signature Changes` 和文案 diff，不能只处理 `New Node Candidates`。

### 10. 把 pin 名当成用户文档说明

输出 pin 名通常是给编译器和 `.gia` 连线用的短标识，不一定适合直接进入中文注释。

例如 `Consume Gift Box` 的输出 pin 是 `Consumer?` / `consumer`，类型是 `Boolean`，官方说明是成功消耗时输出 Yes。中文返回说明应写“是否成功消耗礼盒”，而不是“消耗者”。

处理规则：

- pin 名用于 `markPin` 和 diff 对账。
- `description` 和类型用于用户文档说明。
- `description` 为空时，要结合节点语义和类型人工写清楚。
...
1. 新增节点/事件：手工插入候选代码块。
2. 旧节点签名变化：先确认是否是真实 API 变化，再改签名和实现。
3. 项目封装差异：例如 `Generic` vs `T[]` / `dict<K, V>`，通常保留项目封装。
4. 纯文案变化：逐条更新英文和中文说明，不批量覆盖详情文件。

### B2. 文案与注释复核

定义层落地后，额外做一轮文案复核：

- 28 类疑似改名节点：保留旧 API，但按新资源描述更新注释。
- 同名节点：检查是否有说明新增、返回值说明新增、参数说明变化。
- 中文说明：不能按中文资源数组下标覆盖；用英文节点定位后手工翻译或校正。
- 返回值说明：不要直译 pin 名，要表达类型语义。

推荐用只读脚本或临时脚本列出候选，再人工修改源码。临时脚本可以输出“当前首段说明 vs 资源首段说明”，但不要直接批量写 `src/definitions/*`。
