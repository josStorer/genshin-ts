// Browser-console reference for tests/complex_logic_compare_test.ts.
// Paste this whole file into the browser console. It prints one value per line.
;(() => {
  const ComplexLogicVars = {
  ...
  const list = (_type, values = []) => [...values]
  const dict = (entries) => new Map(entries.map(({ k, v }) => [k, v]))
  const idx = (value) => Number(value)
  const int = (value) => BigInt(value)
  ...
  const customVariables = new Map()
  const asGeneric = (value) => ({
    asType: () => value,
    asDict: () => value
  })
  const runtimeSelf = {
    getCustomVariable(variableName) {
      return asGeneric(customVariables.get(variableName))
    }
  }
  const f = {
    getSelfEntity() {
    ...
    setCustomVariable(_targetEntity, variableName, variableValue) {
      customVariables.set(variableName, variableValue)
    },
    getCustomVariable(_targetEntity, variableName) {
      return asGeneric(customVariables.get(variableName))
    },
    get(variableName) {
