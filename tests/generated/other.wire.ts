...
g.server({ id: 1073741842 }).on('whenEntityIsCreated', (_evt, f) => {
  ...
  f.changePlayerClass(e, vConfig, E.ExistingSkillHandling.ClearAll)
  ...
  f.addCharacterSkill(e, vConfig, E.CharacterSkillSlot.NormalAttack, E.OriginalSlotSkillHandling.Destroy)
  ...
  f.refreshNotificationQueue(e, vInt, vInt)
  f.switchCustomMaps(e, vConfig, vBool)
  f.noOfTasksConfigured(e, vInt, vInt)
  f.increaseTaskCount(e, vInt, vInt)
  const taskCount = f.querySpecifiedTaskCount(e, vInt)
  const taskCountText = f.dataTypeConversion(taskCount, 'str')
  f.printString(taskCountText)
  const taskCompleted = f.queryIfSpecifiedTaskIsCompleted(e, vInt)
  const taskCompletedText = f.dataTypeConversion(taskCompleted, 'str')
  f.printString(taskCompletedText)
})
