...
g.server({ id: 1073741842 }).on('whenEntityIsCreated', (_evt, f) => {
  ...
  f.refreshNotificationQueue(f.getSelfEntity(), 101n, 202n)
  f.switchCustomMaps(f.getSelfEntity(), new configId(303n), true)
  f.noOfTasksConfigured(f.getSelfEntity(), 404n, 505n)
  f.increaseTaskCount(f.getSelfEntity(), 404n, -6n)
  const taskCount = f.querySpecifiedTaskCount(f.getSelfEntity(), 404n)
  const taskCountText = f.dataTypeConversion(taskCount, 'str')
  f.printString(taskCountText)
  const taskCompleted = f.queryIfSpecifiedTaskIsCompleted(f.getSelfEntity(), 404n)
